...
const handleUkloniProizvod = (proizvodId) => {
  ...
  const handleStvoriNarudzbu = () => {

    var today = new Date();
    var date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();

    let helpNacinPlacanja = 1;
    state.naciniPlacanja.forEach(np=>{
      if(np.naziv === formState.kodZaPopust){
        helpNacinPlacanja=np.id
      }
    });

    const data =     {
      datum: date,
      ukupnaCijenaBezP: ukupnaCijena,
      kodZaPopustId: null,
      nacinPlacanjaId: 1,
      brojKartice: '324asdasr3er',
      email: 'helloo@gmail.com',
      brojMobitela: 432244,
      adresaDostave: "Bregovita 9",
