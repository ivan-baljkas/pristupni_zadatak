...
const handleUkloniProizvod = (proizvodId) => {
  ...
  const handleStvoriNarudzbu = (event) => {
         ...
         .then((data) => {
           ...
           data.forEach(np=>{
             if(np.naziv === formState.placanje){
              setState({...state, helpNacinPlacanjaId:np.id})          
             }});
    ...
    const data =  {
      ...
      kodZaPopustId: kodId,
      nacinPlacanjaId: state.helpNacinPlacanjaId,
    ...
    fetch('http://localhost:8080/api/narudzba', {
      ...
      console.error('Error:', error);
        ...
        setState({...state,
          kodovi: state.kodovi.filter(kod=> kod.kod !== formState.kodZaPopust),
