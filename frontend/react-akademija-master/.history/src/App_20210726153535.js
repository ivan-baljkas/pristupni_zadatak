...
useEffect(() => {
  fetch(`http://localhost:8080/api/proizvod`)
    ...
    /*Dohvaćanje Popust kodova*/ 
    fetch(`http://localhost:8080/api/popust-kodovi/kod?iskoristen=${false}`)
         .then((data) => data.json())
         .then((data) => {
...
const handleChange = (event) => {
  ...
  if(formState.email.length>0
      && formState.brojMobitela.length>0
      && formState.adresaDostave.length>0){
        setState({...state, uvjetDisabled:false})
      }
  };
...
const handleUkloniProizvod = (proizvodId) => {
  ...
  const handleStvoriNarudzbu = (event) => {
    ...
    if(formState.placanje==='GOTOVINSKO'){
      helpNacinPlacanjaId=2;
    }

    let helpKodId=0;
    state.kodovi.filter(kod=> kod.kod === formState.kodZaPopust).map(kod=>kod.kod)
                .forEach(kodText=>helpKodId=kodText);


    //const kodId = helpKodId !== 0? helpKodId: null;

    console.log(helpKodId)

    const data =  {
      datum: date,
      ukupnaCijenaBezP: state.ukupnaCijena,
      ukupnaCijenaSP: state.ukupnaCijena,
    ...
    fetch('http://localhost:8080/api/narudzba', {
      ...
      console.error('Error:', error);
        ...
        setState({...state,
          kodovi:state.kodovi.filter(kod=> kod.id !== helpKodId),
    ...
    <div>
      ...
      <SearchForm state={state}  handleSearch={handleSearch} handleChange={handleChange}/>
        ...
        <NarudzbaForm ukupnaCijena={state.ukupnaCijena} naciniPlacanja={state.naciniPlacanja} handleChange={handleChange}
                      handleStvoriNarudzbu={handleStvoriNarudzbu} uvjetDisabled={state.uvjetDisabled}/>
