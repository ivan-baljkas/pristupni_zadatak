...
const handleUkloniProizvod = (proizvodId) => {
  ...
  const handleStvoriNarudzbu = (event) => {
    ...
    let helpKodId;
    /*Dohvaćanje Nacina placanja*/ 
    fetch(`http://localhost:8080/api/popust-kodovi/kod/${formState.kodZaPopust}`)
         .then((data) => data.json())
         .then((data) => {
              helpKodId= data.id
           console.log(helpKodId)
           setState({...state, kodId:parseInt(data.id)})
         })
         .catch((error)=>{
             console.log('Error: ',error);
         });
